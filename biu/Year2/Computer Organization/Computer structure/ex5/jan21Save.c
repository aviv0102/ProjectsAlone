...
/*
	...
	*/
	...
	//charsToPixels(image, pixelsImg)-> reading from image to pixelImg
	pixel* pixels=pixelsImg;
	pixel* p= pixels;	//using pointers 
	int row, col;

	int help2=0;
	p+=n-1;
	
	//going from m-1 to zero because its easier to compare to zero
	for (row =m-1 ; row >=0 ; --row) {
		for (col = n-1; col >= 0; --col) { //same
			help2+=(col<< 1) + (col << 0); //3*col
			p->red = image->data[help2];
			p->green = image->data[help2 + 1];
			p->blue = image->data[help2 + 2];
			--p;
			help2-=(col<< 1) + (col << 0); //3*col
		}
		p+=n; //for col
	...
	//copyPixels(pixelsImg, backupOrg)-> copy the picture to backUp
	pixel *dst=backupOrg;
	pixel *src=pixelsImg;
	pixel *p1 =dst;		//using pointers arithmethic 
	pixel *p2=src;
	p1+=n-1;
	p2+=n-1;
	//going to zero agian...
	...
	for (i =start ; i <end; ++i) {
		...
		//Start of apply Kernel
			...
			*/
			...
			p1=p1+calc2+Max2;
			int red=0,green=0,blue=0; //using int's instead of pixel_sum
	...
	//pixelsToChars(pixelsImg, image)->copy the blur image to image
	pixels=pixelsImg;
	p= pixels;
	help2=0;
	p+=n-1;
	//same as before going to zero
	for (row =m-1 ; row >=0 ; --row) {
		for (col = n-1; col >= 0; --col) {
			//pointers arithmethic
			help2+=(col<< 1) + (col << 0); //3*col
			image->data[help2] = p->red;
			image->data[help2+ 1] = p->green;
			image->data[help2 + 2] = p->blue;
			--p;
			help2-=(col<< 1) + (col << 0); //3*col
		}
		p+=n; //for col
