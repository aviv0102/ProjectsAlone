...
/*
 * initializing the server and starting the server thread
 */
void Server::start(){
    string temp;
    int clientSocket1 = 0;
    ...
    int key = 0;
    Param* p= new Param;
    p->threadPool=new ThreadPool(THREADS_NUM);
    p->socketAdd=serverSocket;

    pthread_create(&serverThread, NULL, acceptClients, (void *)p);
    ...
    GameMap::getInstance()->closeGames();
    p->threadPool->terminate();
    delete(p->threadPool);
    stop(serverThread);
}
/*
 * waiting for clients to connect.
 * @param serverSocket the main socket of the server
 * @return none
 */
void* Server::acceptClients(void * p) {
    Param* param= (Param*)p;
    //creating the ThreadPool
    int clientsCounter = 0;
    ThreadPool* pool=param->threadPool;
    struct sockaddr_in clientAddress;
    socklen_t clientAddressLen;
    int clientSocket1 = 0;

    int server = param->socketAdd;
    while (true) {
        ...
        clientsCounter++;
        //adding threadPool 5 threads
        Task* t = new Task(handleClient, clientSocket1);
        pool->addTask(t);
    }
    //delete pool and task;
}
...
/*this function role is to take a command from the client
 * and take care of it(thread for each client)
 */
void* Server::handleClient(void * client) {
    ...
    while (!inGame) {
        ...
        int j = 0;
        char *param;
        //spliting the string to command and other args
        while (buffer[i] != '\0') {
            if (flag && buffer[i] != ' ') {
                command[i] = buffer[i];
            }
        ...
        args->push_back(param);
        //each thread have the command maneger on his turn...
