...
int main(int argc,char* argv[]) {
    ...
    //setting alarm for movement and apply time passed
...
void play(){
    ...
    //creating Shapes
    myShapes=(shape**)calloc(1,sizeof(shape*));
    count=0;
    if(myShapes==NULL){
        write(2, allocError, sizeof(allocError) - 1);
        return;
    }
    myShapes[count]=CreateShape();
    count++;
    ...
    while(flag){
        ...
        if(change!='u'){
            flag=applyChange(myShapes,count);
            if(flag==0){
                system("clear");
                continue;
            }
        ...
        drawBoard(myShapes,count);

        applyTime=1;
        alarm(1);
        while(applyTime){};
        TimePassed(myShapes,&count,1);


        //if time passed and the shape was deleted create a new one
        if(count==0){
            count++;
            myShapes[count-1]=CreateShape();
        }
    }
 ...
 */
void TimePassed(shape** s,int* n,int flag){
    int i=0;
    int update=*n;
    for(i=0;i<*n;i++){
        if(flag){
