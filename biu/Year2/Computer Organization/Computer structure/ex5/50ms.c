	...
	for (i =start ; i <end; ++i) {
		...
		//Start of apply Kernel
			...
			*/
			...
			current_pixel.red = (unsigned char) (sum.red); 		//sum.x will always be >0 and <=255
			...
			for(index1 = Max1; index1 <= Min1; ++index1) {
				...
				for(index2 = Max2; index2 <= Min2; ++index2) {
					...
					*/
					...
					sum.red += ((int) (*p1).red) * weight;
					sum.green += ((int)(*p1).green) * weight;
					sum.blue += ((int) (*p1).blue) * weight;
					//end of function
				}
				//index* dim
				p1=p+dim;
